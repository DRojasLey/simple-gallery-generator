...
const generateHtmlFile = async (
  ...
  return new Promise((resolve, reject) => {
    ...
    const htmlContent: string = htmlTemplate(title, mainTitle);
    
    // Write the HTML file
    fs.writeFile(filePath, htmlContent, (err: ?Error) => {
      if (err) {
...
const generateCssFile = async (
    ...
    return new Promise((resolve, reject) => {
        ...
        const cssContent: string = cssTemplate(thumbSize);
        // Ensure the directory exists
        const dir: string = path.dirname(filePath);
        if (!fs.existsSync(dir)) {
            fs.mkdirSync(dir, { recursive: true });
        ...
        fs.writeFile(filePath, cssContent, (err: ?Error) => {
            if (err) {
