    ...
    try {
      ...
      setTimeout(() => {
        copyListedImages()
          ...
          .catch(reject) // Reject if copyListedImages fails
      }, 5000)
    } catch (error) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const readImageFile = (imgListFile) => {
      ...
      try {
        ...
        throw error // This ensures the function doesn't return a value if an error occurs
      }
    }

    const images = readImageFile(imageListFile)
    const imageLibrary = {}
